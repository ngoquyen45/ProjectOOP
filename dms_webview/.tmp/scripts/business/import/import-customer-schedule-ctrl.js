(function() {
  'use strict';
  angular.module('app').controller('ImportCustomerScheduleCtrl', [
    '$scope', 'CtrlUtilsService', 'CtrlInitiatorService', 'CtrlCategoryInitiatorService', 'LoadingUtilsService', '$filter', 'toast', '$state', '$stateParams', 'ADDRESS_BACKEND', '$token', function($scope, CtrlUtilsService, CtrlInitiatorService, CtrlCategoryInitiatorService, LoadingUtilsService, $filter, toast, $state, $stateParams, ADDRESS_BACKEND, $token) {
      var importCustomerSchedule, initStep, verify;
      ...
      $scope.isNextDisabled = function() {
        if ($scope.canNext()) {
          if ($scope.currentStep === ($scope.steps.length - 4)) {
            return $scope.global.distributorId === null;
          } else if ($scope.currentStep === ($scope.steps.length - 3)) {
      ...
      $scope.getTemplateLink = function() {
        return ADDRESS_BACKEND + $scope.who + '/import/customer-schedule/template?' + 'distributorId=' + $scope.global.distributorId + '&access_token=' + $token.getAccessToken() + '&lang=' + CtrlUtilsService.getUserLanguage();
      };
      ...
      importCustomerSchedule = function() {
        var onSuccess, params, status;
        $scope.global.result = {};
        status = $scope.loadStatus.getStatusByDataName('data');
        params = {
...
//# sourceMappingURL=import-customer-schedule-ctrl.js.map
